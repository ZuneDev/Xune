...
namespace Microsoft.Iris.Render.Extensions
{
  public static class ImageLoader
  {
    public static bool LoadHeader(IntPtr rgData, int length, out ImageHeader header)
    {
      ...
      return ImageLoader.LoadHeader(rgData, length, req, out header);
    }
      ...
      out GradientInformation gradientInfo)
    {
      ...
      ImageLoader.ValidateRamp(verticalRamp);
      ImageLoader.ValidateRamp(horizontalRamp);
      ...
      for (int index = 0; index < sizeImage.Height; ++index)
      {
        ...
        {
          float flValue = sizeImage.Height > 1 ? (float) index / (float) (sizeImage.Height - 1) : 0.0f;
          colorF1 = ImageLoader.SampleGradient(verticalRamp, flValue);
          colorF1.Clamp();
        }
        int num4 = 0;
        int num5 = 0;
        while (num4 < sizeImage.Width)
        {
          ...
          {
            float flValue = sizeImage.Width > 1 ? (float) num4 / (float) (sizeImage.Width - 1) : 0.0f;
            colorF2 += ImageLoader.SampleGradient(horizontalRamp, flValue);
            colorF2.Clamp();
          }
          switch (nFormat)
          {
            case SurfaceFormat.A8:
              numArray[num3 + num5] = (byte) ((double) colorF2.A * (double) byte.MaxValue);
              break;
            case SurfaceFormat.RGB32:
              numArray[num3 + num5] = (byte) ((double) colorF2.B * (double) byte.MaxValue);
              numArray[num3 + num5 + 1] = (byte) ((double) colorF2.G * (double) byte.MaxValue);
              numArray[num3 + num5 + 2] = (byte) ((double) colorF2.R * (double) byte.MaxValue);
            ...
            case SurfaceFormat.ARGB32:
              ...
              numArray[num3 + num5 + 1] = (byte) ((double) colorF2.G * (double) byte.MaxValue);
              numArray[num3 + num5 + 2] = (byte) ((double) colorF2.R * (double) byte.MaxValue);
              numArray[num3 + num5 + 3] = (byte) ((double) colorF2.A * (double) byte.MaxValue);
              break;
            default:
      ...
      GradientInformation gradientInformation = new GradientInformation()
      {
        ...
        gcData = GCHandle.Alloc((object) numArray, GCHandleType.Pinned)
      };
      ...
      {
        gradientInfo = (GradientInformation) null;
      ...
      float flValue)
    {
      ...
      while (index < rampValues.Length && (double) flValue > (double) rampValues[index].position);
      if (rampValues.Length <= index)
        return rampValue1.value;
      ImageLoader.GradientValue rampValue2 = rampValues[index];
      float num = (float) (((double) flValue - (double) rampValue1.position) / ((double) rampValue2.position - (double) rampValue1.position));
    ...
    private static void ValidateRamp(ImageLoader.GradientValue[] rampValues)
    {
      ...
      {
        Debug2.Validate((double) rampValues[index].position >= (double) num, typeof (ArgumentException), "Gradient ramp values must be monotomically increasing");
        Debug2.Validate((double) rampValues[index].position >= 0.0 && (double) rampValues[index].position <= 1.0, typeof (ArgumentException), "Gradient ramp values must be [0, 1]");
