...
namespace Microsoft.Iris.Render.Protocols.Splash.Rendering
{
    internal class LocalAnimationCallback : RemoteObject
    {
        ...
        internal static RENDERHANDLE BindCallback(RenderPort port) => port.RegisterCallback(new PortCallback(LocalAnimationCallback.DispatchCallback), out uint _);
          ...
          CallbackMessage* message)
        {
            if (!(owner is IAnimationCallback _priv_target) || message->nMsg != 0U)
                return;
            LocalAnimationCallback.Dispatch_AsyncNotify(port, _priv_target, (LocalAnimationCallback.Msg0_AsyncNotify*)message);
        }
          ...
          LocalAnimationCallback.Msg0_AsyncNotify* _priv_pmsg)
        {
            if (_priv_port.ForeignByteOrder)
                MarshalHelper.SwapByteOrder((byte*)_priv_pmsg, ref LocalAnimationCallback.s_priv_ByteOrder_Msg0_AsyncNotify, typeof(LocalAnimationCallback.Msg0_AsyncNotify), sizeof(CallbackMessage), 0);
