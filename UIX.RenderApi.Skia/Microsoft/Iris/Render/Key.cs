...
namespace Microsoft.Iris.Render
{
    internal static class Key
    {
        ...
        private static readonly byte[] s_arPublicKeyBinary = Key.ComputeBinaryKey();

        public static byte[] PublicKeyBinary => (byte[])Key.s_arPublicKeyBinary.Clone();
        ...
        private static uint ParseNibble(char c)
        {
            if (c >= '0' && c <= '9')
                return (uint)c - 48U;
            if (c >= 'a' && c <= 'f')
                return (uint)(10 + ((int)c - 97));
            if (c >= 'A' && c <= 'F')
                return (uint)(10 + ((int)c - 65));
